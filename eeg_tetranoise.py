...
NUM_PARTICLES  = 20000
PARTICLE_SPEED = 0.3 #fix this mess
DT             = 0.016
...
@ti.kernel #now accepting a speed scalar
...
def main():
    ...
    BASE_DT = 0.012
    speed_smooth = 0.001   # start calm
    last_time = time.perf_counter()
    tau_rise  = 0.25       # faster response when stress increases
    tau_fall  = 0.25       # slower when relaxing (feels nicer)
    ...
    while window.running:
        ...
        target_speed = map_ratio_to_speed(ratio, r_min=0.5, r_max=9.0,
                                  v_min=0.001, v_max=0.050)

        # Asymmetric smoothing feels good
        tau = tau_rise if target_speed > speed_smooth else tau_fall
        speed_smooth = exp_smooth(speed_smooth, target_speed, real_dt, tau)
        # Safety clamp
        speed_smooth = float(np.clip(speed_smooth, 0.0005, 0.08))
        # Step sim with fixed dt + smoothed speed
        advect_and_splat(t, BASE_DT, SPLAT_RADIUS, speed_smooth)
